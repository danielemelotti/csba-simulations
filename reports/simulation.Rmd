...
**Which measure of central tendency will be more sensitive to outliers being added to data?** If we were to add outliers to our data, _the mean would be affected the most_. That is because the calculation of the mean is made by summing all the values in a dataset and then dividing by the number of values. Outliers are significantly lower or significantly higher than the rest of the data, hence, they can influence the mean value. On the other hand, the median is the middle value when a data set is ordered from lowest to highest. It divides the dataset into two equal halves. Therefore, it is less sensitive to outliers because it doesn't take into account the magnitude of each value, and the likelihood that adding a few outliers to the dataset will change its value is lower than in the case of the mean. It's only concerned with the order of the data.
